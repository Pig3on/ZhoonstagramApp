describe('Example', () => {
  ...
  it('should be able to register successfully', async () => {
    ...
    await element(by.id('RegisterEmail')).typeText('someone@test.com');
    await element(by.id('RegisterPassword')).typeText('password');
    await element(by.id('RegisterButton')).tap();

    await waitFor(element(by.id('LoginScreen'))).toExist().withTimeout(2000);
  });
  ...
  it('should be able to login successfully', async () => {
    ...
    await element(by.id('LoginEmail')).typeText('someone@test.com');
    await element(by.id('LoginPassword')).typeText('password');
    await element(by.id('LoginButton')).tap();

    await waitFor(element(by.id("FeedScreen"))).toExist().withTimeout(10000);
  });
});
